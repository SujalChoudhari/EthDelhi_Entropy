...
export function saveChat(chat: { id: string, messages: Array<CoreMessage> }) {
  const chats = getLocalStorage('chats') || [];
  const existingChatIndex = chats.findIndex((c: any) => c.id === chat.id);
